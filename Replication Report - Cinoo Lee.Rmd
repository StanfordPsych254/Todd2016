...
Data will be excluded if less than 50% correct and also we will only analyze data from people who indicated their race as White.
...
#FIGURE OUT HOW TO filter out participants who have less than 50% accuracy
assess_accuracy  <- d %>%
  group_by(workerid) %>%
  summarise(mean_accuracy = mean(accuracy))

d.complete <- d %>%
   filter(!(is.na(d$rt)) & accuracy == 1 & rt > 100 &  rt <= 500) %>%
...
#install.packages("ez")
library(ez)
#d <- filter(d, !(is.na(d$rt)))

aov <- ezANOVA(data = d.complete, dv = log_rt, wid = workerid, within = .(targetRace, toolType), detailed = TRUE)
